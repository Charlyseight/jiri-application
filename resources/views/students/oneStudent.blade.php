...
@section ('content')
    ...
    <ul>
            @foreach($student->implementsForCurrentJiriWithProjectAndScore as $implement)
                <li>
                    ...
                    @else
                        <a href="/student/{{ $student->id }}/project/{{ $implement->project->id }}" class="btn btn-primary">
                            Donner une note
                        </a>
                    @endif
                </li>
    ...
    @if($impressionForCurrentJiri->first())
        <p>La note d'appréciation pour tous les projects : {{ $impressionForCurrentJiri->first()->impression_score }}</p>
        ...
        <form action="/impression/{{ $impressionForCurrentJiri->first()->id }}" method="post">
            ...
            <br>
            <input type="number" name="impression_score" id="impression_score" value="{{ $impressionForCurrentJiri->first()->impression_score }}">
            <span>sur 20</span>
            <br>
            <label for="impression_comment">Commentaires</label>
            <br>
            <textarea name="impression_comment" id="impression_comment" cols="30" rows="10">{{ $impressionForCurrentJiri->first()->impression_comment }}</textarea>
            <br>
